...
# 1. Page configuration optimized for mobile and professional dark UI
st.set_page_config(
...
# 2. Custom styled CSS for modern dark theme and professional font scales
st.markdown("""
<style>
...
def get_stock_details(ticker):
    try:
        ...
        # محاولة الجلب السريع للسعر المباشر لضمان عدم توقف الواجهة
        if stock.info and 'regularMarketPrice' in stock.info and stock.info['regularMarketPrice'] is not None:
            ...
            # خيار احتياطي أمان في حال وجود حظر سحابي على خوادم ياهو فاينانس
        ...
        # تأمين التطبيق تماماً من الانهيار السحابي وتوفير محاكاة مستقرة
        fallback_prices = {"NVDA": 208.64, "AAPL": 175.50, "TSLA": 180.20, "AMZN": 178.00, "MSFT": 420.00}
        price = fallback_prices.get(ticker, 150.00)
